...
public class Cert {
    public static CertData unmarshalByte(List<Byte> certList){
        // Search for Public Key prime256v1 OID
        String[] prime256v1_oid_string = new String[]{"0x06",
                "0x08", "0x2a", "0x86", "0x48", "0xce", "0x3d", "0x03", "0x01", "0x07"};

        List<Byte> prime256v1_oid = Utils.string2BytesList(prime256v1_oid_string);

        int offset = Utils.getIndexOf(certList,prime256v1_oid);
        offset += 11; // 10 + TAG (0x03)

        // Obtain Public Key length
        int length = Byte.toUnsignedInt(certList.get(offset));
        if (length > Byte.toUnsignedInt(Utils.hexToByte("80"))){
        ...
        // Obtain Public Key
        offset += 1;
        byte[] pub_k = Utils.list2array(certList.subList(offset+2,offset+length)); // skip "00 04"

        String[] ns_cmt_oid_string = new String[]{"0x06",
                "0x09", "0x60", "0x86", "0x48", "0x01", "0x86", "0xf8", "0x42", "0x01", "0x0d"};
        List<Byte> ns_cmt_oid = Utils.string2BytesList(ns_cmt_oid_string);
        offset = Utils.getIndexOf(certList,ns_cmt_oid);
        ...
        return new CertData(payload,pub_k);
    }

    public static byte[] verifyCert(List<Byte> payload){
        Base64.Decoder decoder = Base64.getDecoder();

        int startIndex = payload.indexOf(Utils.hexToByte("7c"));
        int endIndex = payload.lastIndexOf(Utils.hexToByte("7c"));
        byte[] attnReportRaw = Utils.list2array(payload.subList(0,startIndex));
        byte[] sigRaw = Utils.list2array(payload.subList(startIndex+1,endIndex));
        ...
        byte[] sigCertRaw = Utils.list2array(payload.subList(endIndex+1,payload.size()));
